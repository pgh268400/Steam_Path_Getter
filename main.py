...
def parse_library_vdf(vdf_file_path : str):
...
def parse_appinfo_vdf(appinfo_path : str, library_data : list) -> dict[Any, Any]:
    # library_data 에서 apps 를 모두 가져온다
    app_id_lst : list[str] = []
    for element in library_data:
        app_id_lst.extend(element['apps'])

    result = run_process(["VDFparse.exe", appinfo_path, *app_id_lst])
    # result를 json 형식으로 변환
    json_dict : dict = json.loads(result)
    return json_dict

# app id 리스트를 받아서 acf 파일을 파싱하는 함수
def parse_acf(app_id_lst : list[str]) -> list[dict]:
    acf_lst : list[dict] = []

    for app_id in app_id_lst:
        acf_file_path = os.path.join(install_path, steam_apps, app_manifest.format(app_id))
...
# 가장 먼저 스팀 설치 경로를 읽어온다
install_path : str = steam_install_path_read()
print(f"스팀 설치 경로 : {install_path}")

# libraryfolders.vdf를 파싱해서 설치된 스팀 게임들의 app 번호를 리스트로 반환한다
library_path = os.path.join(install_path, steam_apps, library_folders)
library_data = parse_library_vdf(library_path)
pprint(library_data)


# 스팀 설치 경로 + appcache 안에 appinfo.vdf를 파싱한다
appinfo_path = os.path.join(install_path, app_cache, appinfo)
app_info_dic = parse_appinfo_vdf(appinfo_path, library_data)
