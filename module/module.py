...
from type.type import GameDirData, SteamLibrary
...
class SteamPath:
    ...
    appinfo = (
        "appinfo.vdf"  # 스팀 설치 경로의 appcache 폴더 안에 있는 appinfo.vdf 파일 이름
    )

    app_manifest = (
        "appmanifest_{0}.acf"  # 스팀 설치 경로의 steamapps 폴더 안에 있는 acf 파일 이름
    )
    ...
    def __init__(self) -> None:
        ...
        # library, appinfo 경로를 만든다
        self.__library_path: str = os.path.join(
            self.__install_path, SteamPath.steam_apps, SteamPath.library_folders
        )
        self.__appinfo_path: str = os.path.join(
            self.__install_path, SteamPath.app_cache, SteamPath.appinfo
        )
        ...
        self.__app_info_dic = self.parse_appinfo_vdf(
            self.__appinfo_path, self.__library_data
        )

        # library 데이터와 appinfo 데이터를 합쳐서 게임 디렉토리 경로를 만든다
        self.__game_dir_data = self.get_game_dirs(
            self.__app_info_dic, self.__library_data
        )

        #  마지막으로 base_path 순으로 드라이브 알파벳 정렬한다
        self.__game_dir_data = sorted(self.__game_dir_data, key=lambda x: x.base_path)
    ...
    def parse_library_vdf(self, vdf_file_path: str) -> list[SteamLibrary]:
        dic: dict = vdf.load(open(vdf_file_path, encoding="utf-8"))

        inner: dict = dic["libraryfolders"]

        result: list[SteamLibrary] = []

        for key, value in inner.items():
            base_path: str = value["path"]
            apps: list[str] = list(value["apps"])

            result.append(SteamLibrary(base_path=base_path, apps=apps))
        return result

    # appinfo.vdf 파일을 딕셔너리로 파싱하는 함수
    def parse_appinfo_vdf(
        self, appinfo_path: str, library_data: list[SteamLibrary]
    ) -> dict[Any, Any]:
    ...
    def get_game_dirs(self, app_info_dic, library_data: list[SteamLibrary]) -> list[GameDirData]:
        result: list[GameDirData] = []
        ...
        for element in app_info_dic["datasets"]:
            ...
            for key, value in config["launch"].items():
                ...
                if (
                    oslist == "windows"
                    or "exe" in executable
                    or "bat" in executable
                    or "cmd" in executable
                ):
                    full_path = os.path.join(
                        base_path,
                        SteamPath.steam_apps,
                        SteamPath.common,
                        installdir,
                        executable,
                    )
    ...
    @property
    def app_info_dic(self) -> dict[Any, Any]:
        return self.__app_info_dic

    @property
    def game_dir_data(self) -> list[GameDirData]:
